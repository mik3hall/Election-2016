...
ev$diff.04 <- ev$dem04-ev$rep04
ev$diff.08 <- ev$dem08-ev$rep08
...
```{r state probs, echo=FALSE}
# Need to calculate at state level for '04 and '08 - not included in ev
d04 <- data04 %>% group_by(State) %>% summarize(sum(dem.Count04 - gop.Count04))
names(d04) <- c("State","diff.04")
ev$diff.04 <- d04$diff.04
d08 <- data08 %>% group_by(State) %>% summarize(sum(dem.Count08 - gop.Count08))
names(d08) <- c("State","diff.08")
ev$diff.08 <- d08$diff.08
probs <- c()
for (state in ev$State) {
  row <- ev[ev$State==state,]
  values <- c(row$diff.04,row$diff.08,row$diff.12,row$diff.16)
  diffs <- c(values[2]-values[1],values[3]-values[2],values[4]-values[3])
  if (row$diff.16 > 0) {  # Compute flip to rep odds
    r.cnt <- sum(diffs < 0)
    p.dir <- r.cnt/3
    p.dist <- sum(abs(diffs) > abs(row$diff.16))/3
    probs <- c(probs,p.dir*p.dist)
  }
  else {      # Compute flip to dem odds
    d.cnt <- sum(diffs > 0)
    p.dir <- d.cnt/3
    p.dist <- sum(abs(diffs) > abs(row$diff.16))/3
    probs <- c(probs,p.dir*p.dist*100)
  }
}
# mean.d12 <- sprintf("%.2f",mean(counties12$dem.Count12))

state.probs <- data.frame(cbind(as.character(ev$State),sprintf("%.3f",probs)))
names(state.probs) <- c("State","Probs")
state.probs <- arrange(state.probs,-probs)
head(state.probs,10)
```
...
```{r sampling monte carlo, echo=FALSE}
N <- 50        # Number of iterations
cat("Monte Carlo")
flipped <- data.frame(cbind(as.character(ev$State),rep(0,50),rep(0,50),rep(0,50)))
names(flipped) <- c("State","flipped.dg","flipped.gd","flipped")
flipped$flipped.dg <- as.integer(as.character(flipped$flipped.dg))
flipped$flipped.gd <- as.integer(as.character(flipped$flipped.gd))
flipped$flipped <- as.integer(as.character(flipped$flipped))
for (i in 1:N) {
  idx <- sample(1:4,nrow(data16),replace=T)
  dnew <- c()
  for (j in 1:nrow(data16)) {
    if (idx[j] == 1) {
      dnew <- c(dnew,data04$dem.Count04[j]-data04$gop.Count04[j])
    }
    else if (idx[j] == 2) {
      dnew <- c(dnew,data08$dem.Count08[j]-data08$gop.Count08[j])
    }
    else if (idx[j] == 3) {
      dnew <- c(dnew,data12$dem.Count12[j]-data12$gop.Count12[j])
    }
    else {
      dnew <- c(dnew,data16$dem.Count16[j]-data16$gop.Count16[j])
    }
  }
  df <- data.frame(cbind(as.character(data16$State),dnew))
  names(df) <- c("State","Sim")
  df$Sim <- as.integer(as.character(df$Sim))
  df.state <- df %>% group_by(State) %>% summarize(sum(Sim))
  names(df.state) <- c("State","Diff")
  fdg <- as.numeric(ev$diff.16 > 0 & df.state$Diff < 0)
  flipped$flipped.dg <- flipped$flipped.dg + fdg
  fgd <- as.numeric(ev$diff.16 < 0 & df.state$Diff > 0)
  flipped$flipped.gd <- flipped$flipped.gd + fgd
  flipped$flipped <- flipped$flipped+fdg+fgd
}
flipped$P <- flipped$flipped/50
flipped$EV <- ev$EV
flipped <- arrange(flipped,-flipped)
flipped <- flipped[,c(1:3,5:6)]
print(head(flipped,10))
```
...
```{r past state flips, echo=FALSE}
ev$flips0408 <- state.flips(ev$diff.04,ev$diff.08)
print("2004-2008")
print(table(ev$flips0408))
flipped <- ev[ev$flips0408==-1 | ev$flips0408==1,c("State","EV","diff.04","diff.08")]
print(flipped)
print("2008 Competitiveness...")
margins <- ev[,"diff.08"]
pers <- sprintf("%.4f",margins/(ev[,"dem08"]+ev[,"rep08"]))
comp <- data.frame(cbind(as.character(ev$State),pers))
names(comp) <- c("State","Percentage")
sorted <- order(comp$Percentage)
comp <- comp[sorted,]
comp$Percentage <- as.numeric(as.character(comp$Percentage))*100
head(comp,10)
ev$flips0812 <- state.flips(ev$diff.08,ev$diff.12)
print("2008-2012")
print(table(ev$flips0812))
flipped <- ev[ev$flips0812==-1,c("State","EV","diff.08","diff.12")]
print(flipped)
print("2012 Competitiveness...")
margins <- ev[,"diff.12"]
pers <- sprintf("%.4f",margins/(ev[,"dem12"]+ev[,"rep12"]))
comp <- data.frame(cbind(as.character(ev$State),pers))
names(comp) <- c("State","Percentage")
sorted <- order(comp$Percentage)
comp <- comp[sorted,]
