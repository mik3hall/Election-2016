...
efficiencyGap <- function(dem,rep) {
  ...
  dwin <- dem > rep
  rwasted <- sum(rep[dwin])+sum(rep[!dwin]-(dem[!dwin]+1))
  dwasted <- sum(dem[!dwin])+sum(dem[dwin]-(rep[dwin]+1))
...
eg.wasted
# Actually seems just as easy to calculate, but gets different result
```
